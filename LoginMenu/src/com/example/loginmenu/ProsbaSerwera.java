...
public class ProsbaSerwera {
		...
		public class DaneUzytkownikaAsyncTask extends AsyncTask<Void, Void, Void>{
			...
			@Override
			protected void onPostExecute(Void rezultat) {
		...
		protected Uzytkownik doInBackground(Void... params) {
			...
			Uzytkownik powrotUzytkownika = null;
			...
			try{
				...
				if(jObject.length() == 0){
					powrotUzytkownika = null;
				}else{
					String nazwa = jObject.getString("nazwa");
					
					powrotUzytkownika = new Uzytkownik(nazwa,uzytkownik.login, uzytkownik.haslo, uzytkownik.email);
				}
